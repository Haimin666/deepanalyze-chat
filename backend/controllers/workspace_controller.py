...
工作区控制器
"""
from fastapi import Query, File, UploadFile, HTTPException, Depends
...
class WorkspaceController:
    ...
    async def get_files(
        ...
        """获取文件列表"""
        files = self.workspace_service.get_files(session_id, user_id)
    ...
    async def get_tree(
        ...
        """获取文件树"""
        return self.workspace_service.get_tree(session_id, user_id)
    ...
    async def delete_file(
        ...
        """删除文件"""
        try:
            self.workspace_service.delete_file(path, session_id, user_id)
    ...
    async def delete_dir(
        ...
        """删除目录"""
        try:
            self.workspace_service.delete_dir(path, session_id, user_id, recursive)
            return {"message": "deleted"}
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))
        except FileNotFoundError:
    ...
    async def move_path(
        ...
        """移动文件/目录"""
        try:
            new_path = self.workspace_service.move_path(src, dst_dir, session_id, user_id)
    ...
    async def upload_files(
        ...
        """上传文件到根目录"""
        try:
            saved = self.workspace_service.upload_files(files, session_id, user_id, "")
            return {
    ...
    async def upload_to_dir(
        ...
        """上传文件到指定目录"""
        try:
            saved = self.workspace_service.upload_files(files, session_id, user_id, dir)
    ...
    async def clear_workspace(
        ...
        """清空工作区"""
        try:
            self.workspace_service.clear_workspace(session_id, user_id)
